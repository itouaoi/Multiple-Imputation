...
`mice`パッケージを用いて欠損値の多重代入法を実行する方法をまとめました。はじめに、**基本編として「シングルレベルの多重代入法」**、つぎに**応用編として「マルチレベルの多重代入法」** について説明しています。なお、マルチレベルの多重代入法とは、何らかの階層を持つデータにおける欠損値の多重代入法であり、シングルレベルの多重代入法とは異なるプログラムを組む必要があります。
