...
***
#### maxit: インタラクションの回数
`maxit`は、代入時のインタラクションの回数を指定する引数です。デフォルトでは、5回（`maxit = 5`）で設定されています。 
***
#### m: 代入の回数  
`m`は、代入する回数を指定する引数です。デフォルトは5回（`m = 5`）で設定されています。  
***
# 応用編「マルチレベルの多重代入法」
作成中
***
...
**エラー対処法について**  
+ `mice()`の代入が途中で止まる。  
broom.mixedパッケージを呼び出したら解決した。Rのパッケージをアップデートしたら解決したという人も多い。  
