...
### とくに重要な引数
#### method: 各変数の代入方法の指定
`mice()`の引数であるmethodは、各変数に対する代入方法を指定します。指定できる代表的な方法には、**pmm、norm、logreg、polyreg**などがあります。 もし、この引数をとくに操作しなければ、`mice()`は自動的に各変数に対して適切な方法を指定します。しかし、その方法が必ずしも最適な方法かはわからないため注意が必要です。
